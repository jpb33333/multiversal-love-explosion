...
8. **The simulation is the floor, not the ceiling.** The spread/decay/outcome math is not a place for vibes. Tests assert the invariants (deterministic replay, bounded node count, takeoff + burnout thresholds). Run `npm test` before every commit.
9. **Game feel beats math purity.** Once the rules are correct, the constants (spread rate, decay ramp, takeoff target) get tuned by *playing the game*, not by deriving them. `npm run dev`, play through, adjust, repeat.
...
- Module per concept, named for what it does (`spread.ts`, not `sim-utils.ts`).
...
- **No scattered magic numbers — add to the right table:** simulation constants in `src/sim/constants.ts` (`SIM`, `SPREAD`, `DECAY`, `IGNITE`, `SPAWN`, `TAKEOFF`, `OUTCOME`); win/lose targets in `src/game/outcomes.ts` (`DEFAULT_OUTCOME_CONFIG`); UI limits in `src/game/states.ts` (`LIMITS`).
