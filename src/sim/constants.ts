...
export const CONTAGION = {
  GAIN: 0.9, // neighbor-field strength per second (must beat ENTROPY.BASE)
  ...
  COMMIT_DWELL: 0.5, // … once the threshold has held this many seconds (anti-flicker)
  ...
  NUDGE: 0.9, // love/s a held player caress injects (love-only; up nudges only)
  NUDGE_MAX_PER_TICK: 0.05, // hard cap on a single node's nudge gain per tick (anti-spam)
} as const;

export const ENTROPY = {
  BASE: 0.06, // baseline love drain /s — entropy is the house edge
  RAMP_PER_MIN: 0.05, // added to the drain per elapsed minute (escalation)
  MAX: 0.45, // drain ceiling so late game is brutal, not instant-death
...
export const SPAWN = {
  SEED_COUNT: 14, // nodes placed at t=0 so the first cluster can grow
  SEED_RADIUS: 160, // world radius of the initial seed disc
  RATE_BASE: 3.0, // spawns/s at t=0
  RATE_RAMP_PER_MIN: 2.5, // +spawns/s per elapsed minute
  RATE_MAX: 16.0, // spawns/s ceiling ("too much" target)
  DIST: 95, // distance a new node sits from its anchor node
  DIST_JITTER: 35, // ± jitter on that distance
  MIN_GAP: 60, // reject a spawn closer than this to any existing node
  PLACE_ATTEMPTS: 8, // tries to find an open spot before skipping the spawn
  K_NEIGHBORS: 4, // max proximity edges wired per spawn (bounded degree)
  EDGE_MAX_DIST: 150, // world-unit cap on edge length
  LOVE_JITTER: 0.03, // ± initial love spread for a fresh potential
  LOVE_SEEDED: 0.62, // initial love for a node spawned on a lock-in credit
} as const;

export const LOCK = {
  SCAN_INTERVAL: 0.25, // seconds between connected-component lock scans (cost control)
  MIN_SIZE: 6, // loving-cluster size to auto-lock (the "shape" you built)
  MIN_MEAN_LOVE: 0.85, // cluster must be genuinely loving, not borderline
  STABILITY_SECONDS: 0.8, // every member loving this long (settled, not a fluke)
  ...
  RELIEF: 0.01, // entropy relief /s per node locked (the breather)
  SPAWN_CREDIT: 0.5, // love-seeded frontier spawns granted per node locked
  BANK_WINDOW: 12, // seconds a lock's love counts toward the win love-share
} as const;
